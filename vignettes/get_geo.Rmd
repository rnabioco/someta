...
gds <- readRDS(here("inst", "extdata", paste0("geo_", "101220", ".rds"))) %>% 
...
gds_c_if2 <- gds_j %>% 
  ...
  filter(year == 2017 | year == 2018 | year == 2016) %>% 
  # mutate(year = ifelse(month %in% c("Jan", "Feb", "Mar", "Apr", "May", "Jun"),
  #                      year + 0.5,
  #                      year)) %>%
...
g <- ggplot(gds_c_if2,
  ...
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  coord_cartesian(ylim = c(0,500)) +
  xlab("IF") + ylab("# of citations") +
  ggtitle("citations by IF, 2017/8")
g
