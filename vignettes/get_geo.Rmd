...
#         TRUE ~ "+",
#         )) %>%
#       filter(year == yearcut + 1, month %in% c("Jan", "Feb", "Mar", "Apr", "May", "Jun")) 
#   )
# }
...
g <- ggplot(
  ...
  coord_cartesian(ylim = c(0, 500)) +
  xlab("journal IF") +
  ylab("# of citations") #+ ggtitle("citations by IF, 2017/8")
g
...
ts2 <- ts %>%
  ...
  filter(!is.null(pubmed), !is.na(pubmed)) %>% 
  select(DOI, pubmed) %>%
  na.omit() %>% 
  distinct() %>%
...
sets_10x %>% separate(ReleaseDate, sep = "-", into = c("year", "month", "day")) %>% 
  group_by(year) %>% 
  summarize(frac = sum(str_detect(str_to_lower(samplemetainfo_1), "meta|annot|type|clustering|coldata") | 
    str_detect(str_to_lower(additionalfiles), "meta|annot|type|clustering|coldata")) / n(),
    n = n())
```
