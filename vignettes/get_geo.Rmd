...
mean(gs_data_processed$usable == "yes")
  ...
  function(x) {
    ...
    if (length(temp) == 1) {
      if (is.na(temp)) {
        ...
        length(intersect(temp, gds$id)) == 0
      }
    } else {
      length(intersect(temp, gds$id)) == 0
    }
...
Overlap of scRNA-seq GEO entries between manual curation and the GEO query is `r 1 - sum(checksub, na.rm = T)/(length(gs_ids) - sum(is.na(checksub)))`.
...
ggplot(gds_j %>% filter(year >= 2018) %>% mutate(journal_group = factor(journal_group, levels = gds_j_frac$journal_group)),
       aes(x = journal_group, fill = usable)) +
  geom_bar() +
  scale_fill_viridis_d(option = "plasma") +
  geom_text(data = gds_j_frac, color = "black", aes(x = journal_group, y = total + 30, label = fraction, fill = NA)) +
  cowplot::theme_cowplot() +
  ggtitle("scRNAseq datasets by journal")
```

```{r journaldot}
gds_sum <- gds_j %>% filter(year >= 2018) %>% 
  group_by(journal, journal_group, journal_group2) %>% 
  summarize(n = n(), frac = sum(usable == "yes") / n) %>% 
  ungroup()

ggplot(gds_sum %>% filter(n >= 3) %>% 
